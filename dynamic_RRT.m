classdef dynamic_RRT
    properties
        % nothing  :)
    end
    methods 
        function repeat_dynamic_RRT(obj, map, goal, map_size)
            hold on;
            global path;
            global max_dms;
            met_obs = 0;
        ...
        function path = dynamic_RRT_path(obj, map, start, goal, map_size, old_path)
            global max_dms;
            global path;
            hold on;
            max_dms = 5;
            ...
            while (obj.COST(q_new.data, goal) > max_dms)
                ...
                if (q_new_x > 1 && q_new_x < map_size && q_new_y > 1 && q_new_y < map_size)
                    if (obj.check_valid(map, max_dms, [q_new_x, q_new_y]) == 1)
                        ...
                        plot([q_new_x, q_nearest.data(1)], [q_new_y, q_nearest.data(2)], 'g-+');
                    end
                end
            end
            point = q_new;
